...
#define INDICATOR_TIMEOUT 3000  // 3秒後に消える
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
  ...
  // SOLID_COLOR モードの時のみ全LEDを消灯
  if(rgb_matrix_mode == 1) {
  ...
  // タイマーチェック: 一定時間経過したらインジケーターを非表示
  if (show_indicators && timer_elapsed32(indicator_timer) > INDICATOR_TIMEOUT) {
    show_indicators = false;
  }

  // show_indicatorsがtrueの時のみスピード・ステップインジケーターを表示
  if (show_indicators) {
    RGB rgb_speed = {
